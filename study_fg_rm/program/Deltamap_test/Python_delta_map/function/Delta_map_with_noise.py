...
def make_data_m(cmb, noise, freq_band, which_model, nside):
    ...
    if which_model == "s1":
        ...
        for ii, freq in enumerate(freq_band):
            ...
            # data m[I, Q, U]
            m = fg.fg_make_file(freq, nside)[0] + cmb + noise[0][ii]
        ...
        for ii, freq in enumerate(freq_band):
            ...
            # data m[I, Q, U]
            m = fg.fg_make_file(freq, nside)[1] + cmb + noise[0][ii]
        ...
        for ii, freq in enumerate(freq_band):
            ...
            # data m[I, Q, U]
            m = fg.fg_make_file(freq, nside)[0] + fg.fg_make_file(freq, nside)[1] + cmb + noise[0][ii]
...
# クリーンマップの計算
def calc_clean_map(freq_band, noise_seed_array, data_m, D, nside):
    ...
    for i, freq in enumerate(freq_band):
        ...
        matrices.append(diag * (sigma[i]**2))

    Noise_cov_mat = create_block_matrix(matrices)
    
    A = bslash(Noise_cov_mat, D)
    B = D.T @ A
    C = bslash(B, D.T)
    D = C @ bslash(Noise_cov_mat, diag_2)
    clean_map = D @ data_m
