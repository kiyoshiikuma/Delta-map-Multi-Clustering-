...
function make_data_m(cmb, freq_band, which_model, nside)
    ...
    for (ii, freq) in enumerate(freq_band)
        ...
        # file name
        dir = "/Users/ikumakiyoshi/Library/Mobile Documents/com~apple~CloudDocs/study_fg_rm/program/Deltamap_test/julia_delta-map/fg_map_make/fg_map_data/"
        nside_name = "nside_"
        ...
        if which_model == "s1"
            ...
            data_m[Int((2*(ii-1)+1)*N_pix)+1 : Int((2*(ii-1)+1)*N_pix)+N_pix] = m_U

            #println(Int((2*(ii-1))*N_pix)+1)
            #println(Int((2*(ii-1)+1)*N_pix))
            #println(Int((2*(ii-1)+1)*N_pix)+1)
            #println(Int((2*(ii-1)+1)*N_pix)+N_pix)
